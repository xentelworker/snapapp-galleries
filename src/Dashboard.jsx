 ...
 return <div className="portal">
  ...
  <main className="portal-main">
   ...
   {page==="edit"&&<>
    ...
    {selected&&<section className="portal-panel portal-editor"><h2>Photos <span className="portal-muted">({photos.length})</span></h2>{photos.length>0&&<div style={{margin:"12px 0 18px"}}><p className="portal-muted">Choose a photo for the gallery cover. Then use the slider to move the image up or down inside the cover frame.</p>{selected.cover_photo_id&&<><label>Cover vertical position <input type="range" min="0" max="100" value={selected.cover_position_y??50} onChange={e=>setSelected(s=>({...s,cover_position_y:Number(e.target.value)}))} onPointerUp={async e=>{try{await api("/"+encodeURIComponent(selected.id),{method:"PATCH",body:JSON.stringify({coverPositionY:Number(e.currentTarget.value)})});setNotice("Cover position saved.");}catch(err){setError(err.message);}}} onKeyUp={async e=>{try{await api("/"+encodeURIComponent(selected.id),{method:"PATCH",body:JSON.stringify({coverPositionY:Number(e.currentTarget.value)})});}catch(err){setError(err.message);}}}/><span> {selected.cover_position_y??50}%</span></label><div style={{marginTop:"12px",height:"260px",borderRadius:"12px",overflow:"hidden",position:"relative",background:"#111"}}><img src={"/media/"+encodeURIComponent(selected.cover_photo_id)} alt="Live cover preview" style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"center "+(selected.cover_position_y??50)+"%",display:"block"}}/><div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.30)"}}/><div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"white",textAlign:"center",padding:"24px",textShadow:"0 2px 12px rgba(0,0,0,.55)"}}><small style={{letterSpacing:".14em",fontWeight:700}}>LIVE COVER PREVIEW</small><h2 style={{fontSize:"clamp(28px,4vw,48px)",margin:"8px 0"}}>{selected.title}</h2>{selected.subtitle&&<p style={{margin:0,fontSize:"18px"}}>{selected.subtitle}</p>}</div></div><p className="portal-muted" style={{marginTop:"8px"}}>Preview includes the same dark overlay used on the client gallery.</p></>}</div>}<PhotoUpload key={selected.id} galleryId={selected.id} archived={selected.status==="archived"} onBusy={setBusy} onExpired={onExpired} onComplete={async()=>{try{const data=await api("/"+encodeURIComponent(selected.id));setPhotos(data.photos);setGalleries(rows=>rows.map(g=>g.id===selected.id?data.gallery:g));}catch(e){setError(e.message);}}}/>{photos.length?<div className="portal-photos">{photos.map(p=><a key={p.id} href={"/media/"+encodeURIComponent(p.id)} target="_blank" rel="noreferrer"><img src={"/media/"+encodeURIComponent(p.id)} alt={p.original_filename} loading="lazy"/><span>{p.original_filename}</span><button type="button" className="portal-secondary" style={{marginTop:"6px"}} onClick={async e=>{e.preventDefault();e.stopPropagation();try{const data=await api("/"+encodeURIComponent(selected.id),{method:"PATCH",body:JSON.stringify({coverPhotoId:p.id})});setSelected(data.gallery);setGalleries(rows=>rows.map(g=>g.id===selected.id?data.gallery:g));setNotice("Cover photo updated.");}catch(err){setError(err.message);}}}>{selected.cover_photo_id===p.id?"Current cover":"Set as cover"}</button></a>)}</div>:<div className="portal-empty"><Image size={32}/><h3>No photos yet</h3><p>Use Upload photos above to add your first images.</p></div>}</section>}
   </>}
  </main>
 </div>;
}
